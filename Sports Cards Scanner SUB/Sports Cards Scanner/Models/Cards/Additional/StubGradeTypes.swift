...
extension StubGradeTypes {
    ...
    init?(sciGrade: String) {
        ...
        guard let sciGrade = graderGrade[safe: 1]?.lowercased(),
              let cardGrade = StubGradeTypes(sciValue: sciGrade)
        else {
            return nil
        }
