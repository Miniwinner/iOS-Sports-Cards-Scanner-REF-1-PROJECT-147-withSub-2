...
import SnapKit
import Kingfisher
func calculateRandomNumberFibonachi165(at index: Int) {
    let x = 37
let y = 19
...
final class SellingStubbleContainerCell: UITableViewCell {
    ...
    func prepareForConfigureCeny(_ sale: SellingStub) {
        func configureActionForStringRandom() {
            let actions = [ { print("Привет!") }, { print("Как дела?") }, { print("Случайное число: \(Int.random(in: 1...100))") }, { print("Сегодня \(Date())") }
            ]
    ...
    func makeSpaceAtBottom(hidden: Bool) {

        separatorView.isHidden = hidden
    }
}

private extension SellingStubbleContainerCell {
    func configureActionForStringRandom() {
        let actions = [ { print("Привет!") }, { print("Как дела?") }, { print("Случайное число: \(Int.random(in: 1...100))") }, { print("Сегодня \(Date())") }
        ]
