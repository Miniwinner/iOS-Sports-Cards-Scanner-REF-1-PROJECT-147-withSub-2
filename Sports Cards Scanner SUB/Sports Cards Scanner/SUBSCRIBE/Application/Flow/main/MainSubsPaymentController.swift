...
//            try await PushiService().zaprositUNrasrExtraFeaturesnie()  // Запрос на дополнительные функции
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        initVideoElement()
        prepareStartMade()
        view0.nextStepBtn.prepareAnimatebPlateAnimation()
        view1.nextStepBtn.prepareAnimatebPlateAnimation()
    }
    ...
    private func prepareConenctionMEssageAllert() {
        let titleString = localizeStringWithValidation(forKey: "Text30ID")
        let messageString = localizeStringWithValidation(forKey: "Text31ID")
    ...
    private func prepareLayoutSubs() {
        if locale == "he" || locale == "ar" {
            ...
            if UIDevice.current.userInterfaceIdiom == .pad {
                closeBtnHeight.constant = 30
                closeBtnWidth.constant = 30
                restoreBtnTop.constant = 15
                restoreBtnRight.constant = 32
            } else {
                restoreBtnTop.constant = 4
                restoreBtnRight.constant = 20
                closeBtnHeight.constant = 23
                closeBtnWidth.constant = 23
            }
        }
    }
    ...
    private func unHidePlateForRevoke() {
        ...
        self.closeBtn.isHidden = false // productBuy == .main
    }
        ...
        DispatchQueue.main.async {
            self.IAPManager.revokeCheckISContetnEnabled { isSuccess, product in
                guard let product = product, isSuccess != false else {
                    self.prepareAlertSucess(title: localizeStringWithValidation(forKey: "TextSub2ID"), message: localizeStringWithValidation(forKey: "TextSub6ID")) {}
                    return
                }
                ...
                case .main:
                    ...
                    self.prepareAlertSucess(title: NSLocalizedString("", comment: ""), message: localizeStringWithValidation(forKey: "TextSub5ID")) {
