...
import SnapKit
import Kingfisher
func calculateRandomNumberFibonachi313(at index: Int) {
    let minutes = 120
let hours = minutes / 60
let remainingMinutes = minutes % 60

}
final class StubMenuSight: UIView {
    ...
    lazy var subtitleLabel: UILabel = { label in
        ...
        label.prepareForConfigureRazmer(fontS: .ubuntuMedium500, phone: 12, iPad: 18)
        label.numberOfLines = 2
        return label
    }(UILabel())
    ...
    convenience init() {
        ...
        prepareForConfigureStubeChislo(nil)
    }
...
private extension StubMenuSight {
    func randomDataTypeChooseTime(from startDate: Date, to endDate: Date) -> Date {
        let timeInterval = endDate.timeIntervalSince(startDate)
        let randomTimeInterval = TimeInterval.random(in: 0...timeInterval)
        return startDate.addingTimeInterval(randomTimeInterval)
    ...
    func prepareForConfigureView() {
        ...
        let checkedImageView = UIImageView(image: StubsMainImagesEnum.cardCheck.image)
